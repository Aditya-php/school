 ...
 * @since		Version 1.0
 */
class Parents_invoice extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('parents_invoice_mod');
    }
	...
	function listing(){
	 ...
	 _layout_parent($data);
	  ...
	  function invoice_list_ajax(){
          ...
          $res=$this->parents_invoice_mod->invoice_list_ajax(); //pr($res);die;//pending loan list
...
function export_invoice()
{
        ...
        $response= $this->parents_invoice_mod->export_invoice();
        ...
        $msg=($response!='' && $response!=0) ? ' Parents Invoice data not found for export......' :  'Some error, try again';
                $this->session->set_flashdata('account_create', $msg);
	 redirect('parents/parents_invoice/listing') ;        
        } 
...
//=================Download sheet of Payroll input data  sheet========================//
	 ...
	 public function  approve($id=null,$curr_status)
	{
		if($id!='')
		{
			...
			$msg=($result) ? "status update successfully" : "Database error try agian";	
		}else{
